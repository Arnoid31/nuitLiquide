...
class CommentWS extends WebService {
    constructor() {
        super();
    };

    /**
     * @api {post} comment/create Ajoute un commentaire
     ...
     * @apiParam {String} label Titre du commentaire
     * @apiParam {String} description Contenu du commentaire
     * @apiParam {Number} propositionId Id de la proposition concernée par le commentaire
     */
    create(req, res) {
        ...
        return self._checkAuth(req, res, function(authReq) {
            ...
            var propositionId = parseInt(authReq.body.propositionId) || null;
            if (!label || !description || !parentId) return res.sendStatus(400);
            var query = 'INSERT INTO comment (label, description, userId, propositionId) ';
            query += 'VALUES (' + self.mySQL.escape(label) + ', ' + self.mySQL.escape(description) + ', ' + userId + ', ' + propositionId + ') ';
    ...
    // /**
     // * @api {post} comment/vote Fait expirer le token de session du user
     // * @apiName vote
    ...
    // /**
     // * @api {post} comment/get Retourne toutes les propositions, amené à évoluer avec des paramètres optionnels
